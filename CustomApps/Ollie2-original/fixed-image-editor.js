// Fixed Enhanced NebulaImageEditor with working layer visibility and transparency slider
// This version fixes the layer visibility toggle and adds individual layer transparency controls

class FixedNebulaImageEditor {
    constructor() {
        this.windowId = null;
        this.canvas = null;
        this.context = null;
    ...
    async init() {
        ...
        this.windowId = window.windowManager.createWindow({
            title: 'Nebula Image Editor Pro - Fixed',
            width: 1400,
            height: 900,
            resizable: true,
            maximizable: true,
        ...
        console.log(`Fixed NebulaImageEditor initialized with window ${this.windowId}`);
    }
    
    render() {
        const container = document.createElement('div');
        container.className = 'fixed-image-editor-container';
    ...
    createEnhancedToolbar() {
        ...
        toolbar.innerHTML = `
            ...
            <div class="toolbar-title">
                <span class="app-title">Nebula Image Editor Pro - Fixed</span>
    ...
    createRightPanel() {
        ...
        // Create panels
        const layersPanel = this.createFixedLayersPanel();
    ...
    createFixedLayersPanel() {
        const panel = document.createElement('div');
        panel.className = 'fixed-layers-panel';
        ...
        panel.innerHTML = `
            ...
            <div id="fixed-layers-list" class="fixed-layers-list">
    ...
    addEnhancedStyles() {
        if (document.querySelector('#fixed-image-editor-styles')) return;
        
        const style = document.createElement('style');
        style.id = 'fixed-image-editor-styles';
        style.textContent = `
            ...
            .fixed-layer-item:last-child {
                border-bottom: none;
            }
            
            .fixed-layer-item:hover {
                background: var(--nebula-surface-hover);
            }
            
            .fixed-layer-item.active {
                background: var(--nebula-primary);
                color: white;
            }
            ...
            .layer-control-btn.visibility-off {
                opacity: 0.4;
            }
    ...
    setupEnhancedPropertyControls() {
        ...
        // Layer opacity with proper event handling
        ...
        if (layerOpacity) {
            layerOpacity.addEventListener('input', (e) => {
                ...
                if (activeLayer) {
                    this.layerManager.setLayerProperty(activeLayer.id, 'opacity', value / 100);
    ...
    updateLayersPanel() {
        const layersList = document.getElementById('fixed-layers-list');
        ...
        layers.forEach(layer => {
            ...
            layerItem.className = 'fixed-layer-item';
            ...
            layerItem.innerHTML = `
                ...
                <div class="layer-controls">
                    <button class="layer-control-btn visibility-btn ${layer.visible ? '' : 'visibility-off'}" 
                            title="Toggle Visibility" data-layer-id="${layer.id}">
                        <span class="material-symbols-outlined" style="font-size: 16px;">
                            ${layer.visible ? 'visibility' : 'visibility_off'}
                        </span>
                    </button>
                    <button class="layer-control-btn lock-btn" title="Lock Layer" data-layer-id="${layer.id}">
                        <span class="material-symbols-outlined" style="font-size: 16px;">
        ...
        // Add event listeners for visibility and lock buttons
        layersList.querySelectorAll('.visibility-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const layerId = e.currentTarget.dataset.layerId;
                const layer = this.layerManager.getLayer(layerId);
                if (layer) {
                    this.layerManager.setLayerProperty(layerId, 'visible', !layer.visible);
                    this.updateLayersPanel();
                    this.updateStatus(`Layer ${layer.visible ? 'hidden' : 'shown'}`);
                }
            });
        });
        
        layersList.querySelectorAll('.lock-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const layerId = e.currentTarget.dataset.layerId;
                const layer = this.layerManager.getLayer(layerId);
                if (layer) {
                    this.layerManager.setLayerProperty(layerId, 'locked', !layer.locked);
                    this.updateLayersPanel();
                    this.updateStatus(`Layer ${layer.locked ? 'unlocked' : 'locked'}`);
                }
            });
        });
    ...
    getTitle() {
        return 'Nebula Image Editor Pro - Fixed';
    }
    ...
    cleanup() {
        ...
        console.log('Fixed NebulaImageEditor cleanup completed');
    }
}

// Export for use in NebulaDesktop
window.FixedNebulaImageEditor = FixedNebulaImageEditor;
window.fixedImageEditor = null;

// Register the fixed app with WindowManager
window.fixedImageEditor = new FixedNebulaImageEditor();
