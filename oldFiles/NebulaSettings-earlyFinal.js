class NebulaSettings {
    ...
    applyImportedTheme(themeData) {
        if (themeData.version && themeData.version.includes('NebulaDesktop')) {
            ...
            this.showNotification(`🎨 Theme "${themeData.name || 'Imported Theme'}" applied successfully!`);
            console.log('Theme imported and applied:', themeData);
        } else {
            this.showNotification('❌ Invalid theme file format', 'error');
        }
    }
...
© 2024 NebulaDesktop Project`);
    ...
    showNotification(message, type = 'success') {
        ...
        notification.style.cssText = `
            ...
            top: 20px;
            right: 20px;
            background: ${type === 'success' ? 'var(--nebula-primary)' : type === 'error' ? '#ef4444' : '#f59e0b'};
            color: white;
            padding: 12px 20px;
            border-radius: 8px;
            z-index: 10000;
