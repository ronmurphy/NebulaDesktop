// Enhanced NebulaTerminal.js - Real terminal with file system integration
// Complete rewrite with all syntax errors fixed
...
class NebulaTerminal {
    ...
    render() {
        ...
        terminalDiv.id = `terminal-${this.windowId}`;
     ...
     */
    showPrompt() {
        const prompt = `\nnebula@desktop:${this.getShortPath()}$ `;
        this.terminal.appendChild(document.createTextNode(prompt));
        this.currentInput = '';
    ...
    appendToInput(char) {
        ...
        this.terminal.appendChild(document.createTextNode(char));
        this.terminal.scrollTop = this.terminal.scrollHeight;
    }
    ...
    handleBackspace() {
        if (this.currentInput.length > 0) {
            ...
            this.terminal.textContent = content.slice(0, -1);
            this.terminal.scrollTop = this.terminal.scrollHeight;
        }
    }
    ...
    replaceCurrentInput(newInput) {
        // Remove current input from display
        const content = this.terminal.textContent;
        const lastPromptIndex = content.lastIndexOf('$ ');
        if (lastPromptIndex !== -1) {
            this.terminal.textContent = content.substring(0, lastPromptIndex + 2);
        }
        
        // Set new input
        this.currentInput = newInput;
        this.terminal.appendChild(document.createTextNode(newInput));
        this.terminal.scrollTop = this.terminal.scrollHeight;
    }
    ...
    writeError(text) {
        ...
        this.terminal.appendChild(errorSpan);
        this.terminal.scrollTop = this.terminal.scrollHeight;
    }
  ...
  • Text files open in text editor
    ...
    async listDirectory(path) {
        try {
            ...
            this.terminal.appendChild(listContainer);
            this.terminal.scrollTop = this.terminal.scrollHeight;
