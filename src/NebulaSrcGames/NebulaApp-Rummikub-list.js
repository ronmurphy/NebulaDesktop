class NebulaApp_Rummikub {
  ...
  createDebugSection() {
    ...
    stateDiv.innerHTML = `
      ...
      Turn: ${this.state.turn + 1}/∞ | Tiles left: ${this.state.tiles.length}<br>
  ...
  createWinScreen() {
    ...
    scoreText.innerHTML = `
      Final Scores:<br>
      ${this.state.players.map(p => `${p.name}: ${p.score} points`).join('<br>')}
    `;
  ...
  aiTurn() {
    for (let i = 1; i < this.state.players.length; i++) {
        ...
        for (let handIndex = ai.hand.length - 1; handIndex >= 0; handIndex--) {
          ...
          for (let meldIndex = 0; meldIndex < this.state.melds.length; meldIndex++) {
            ...
            if (this.isValidMeld(testMeld)) {
              ...
              madePlay = true;
              this.state.message += `<br>🤖 AI ${i} added tile to existing meld (+${this.calculateTileValue(tile)} pts)`;
        ...
        if (meld.length >= 3 && this.isValidMeld(meld)) {
          ...
          if (!ai.initialMeld && meldScore < 30) {
            ...
            if (betterMeld.length >= 3 && this.calculateMeldScore(betterMeld) >= 30) {
              ...
              ai.initialMeld = true;
              madePlay = true;
              this.state.message += `<br>🤖 AI ${i} played initial meld (${this.calculateMeldScore(betterMeld)} pts)`;
            } else {
            ...
            this.state.message += `<br>🤖 AI ${i} played a ${meld.length}-tile meld (${meldScore} pts)`;
          }
      ...
      if (!madePlay && this.state.tiles.length > 0 && ai.hand.length < 20) {
        ...
        this.state.message += `<br>🎴 AI ${i} drew a tile`;
      } else if (!madePlay) {
        this.state.message += `<br>⏭️ AI ${i} passed`;
      }
    }
    
    this.state.message += `<br>🎮 Your turn!`;
